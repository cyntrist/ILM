    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback)
            {
                ...
                if (pcmd->UserCallback == ImDrawCallback_ResetRenderState || pcmd->UserCallback == ImGui_ImplSDLRenderer3_DrawCallback_ResetRenderState)
                    ImGui_ImplSDLRenderer3_SetupRenderState(renderer);
                else
...
bool ImGui_ImplSDLRenderer3_Init(SDL_Renderer* renderer)
{
    ...
    IM_UNUSED(ImGui_ImplSDLRenderer3_DrawCallback_SetSamplerLinear);
    IM_UNUSED(ImGui_ImplSDLRenderer3_DrawCallback_SetSamplerNearest);
