...
public class CreateTexturePass : ScriptableRenderPass
{
    ...
    {
        // a�adimos un nodo en el grafo y ese nodo va a recibir en su funci�n de renderizado un pass data
        ...
        using (var builder = renderGraph.AddRasterRenderPass<PassData>("Crear textura de color", out var passData)) // devuelve un constructor de nodo, con esto se abre y luego hay que cerrarlo
        {
            ...
            // frame data es un contenedor de muchas cosas, con el get coges el objeto que sea de ese tipo (solo habr� uno)
            UniversalResourceData resourceData = frameData.Get<UniversalResourceData>();

            // la textura en la que est�bamos escupiendo cosas
            var source = resourceData.activeColorTexture;

            // creamos la textura/target/destino nueva
            // le pedimos al render graph que cree una textura, un recurso que el propio render graph gestionar�, ser� el responsable de su ciclo de vida
            ...
            builder.AllowPassCulling(false); // forzamos al render graph para que no elimine la pasada, ya que si no se usa la pasada el render graph la eliminar� por eficiencia
        }
